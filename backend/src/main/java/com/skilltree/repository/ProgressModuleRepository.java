...
/**
 * Репозиторий для ProgressModule. Добавлен метод для поиска прогресса по
 * module.id и taken_courses.id (конкретное прохождение пользователя).
 */
public interface ProgressModuleRepository extends JpaRepository<ProgressModule, Long> {

	/**
	 * Найти ProgressModule по id модуля и id записи TakenCourses. Используется при
	 * проверке зависимостей (нужно узнать прогресс блокирующего модуля для
	 * конкретного takenCourse).
	 *
	 * @param moduleId
	 *            id модуля (ProgressModule.module.id)
	 * @param takenCourseId
	 *            id записи TakenCourses (ProgressModule.taken_courses.id)
