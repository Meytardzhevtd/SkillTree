...
class DependenciesTestUnit {
	...
	void makeCycleDependence() {
		when(dependencyRepository.findByMainModuleId(1L))
				.thenReturn(List.of(new Dependencies(1L, moduleA, moduleB)));
		when(dependencyRepository.findByMainModuleId(2L))
				.thenReturn(List.of(new Dependencies(2L, moduleB, moduleC)));
